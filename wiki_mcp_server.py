"""
Minimal MCP server exposing a company-wiki search tool -- a simple mock RAG.
Retrieval here is plain keyword overlap (no embeddings, no vector DB) so it runs
with zero setup. Swap `_score` for a real embedding/vector-store lookup to turn
this into production RAG without changing the tool's interface.
"""
import re
...
# Tags mirror DEFAULT_CATEGORIES in support_agent.py, so a wiki hit maps straight
# onto the category classify_email/create_ticket already use -- no translation needed.
ARTICLES = [
    {
        "id": "wiki-001",
...
def search_wiki(query: str, top_k: int = 2) -> List[dict]:
    ...
    scored = [
        (_score(query, a["title"] + " " + a["content"] + " " + " ".join(a["tags"])), a)
        for a in ARTICLES
    ]
