"""
Fixed evaluation set for agent_evaluation.py: 6 synthetic customer emails, one per
DEFAULT_CATEGORIES bucket (see support_agent.py), each with a human-assigned gold
category and priority to check classify_email's predictions against.

Emails 1-4 are written to plausibly match an existing wiki_mcp_server.py article, so
the agent should be able to draft a grounded reply for them. Email 5 asks about
something no wiki article covers (enterprise sales demos), so it should get no reply
-- create_ticket instead. Email 6 is a case where search_wiki *may* return a hit
(wiki-009, escalation policy) but that article is written as internal guidance, not
customer-facing copy -- a good check of whether the agent drafts an appropriate reply
or (correctly) declines to parrot internal policy text at the customer.
"""
